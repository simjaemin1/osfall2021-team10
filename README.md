...
time_before(curr_time, load_balanced_time + WRR_PERIOD)로 현재 시간이 이전에 load balance 한 시간에 비해 2000ms가 지나지 않은 경우 다른 동작을 할 필요가 없으므로 return 한다. time_before macro는 unsigned long의 overflow도 고려했으므로 jiffies의 크기에 관계없이 정상적으로 동작한다.  
위 조건이 맞지 않은 경우(2000ms 이상이 지난 경우) 밑의 과정으로 넘어간다.
